...
const projects = [
  {
    ...
    caseStudy: 'The site translates a compliance-heavy service offering into a confident digital experience. It highlights safety management, audits, documentation, process, client trust and enquiry flow while keeping the tone serious and operational.',
    stack: ['HTML', 'CSS', 'JavaScript', 'GitHub Pages', 'Responsive UI', 'Contact Forms'],
    color: '#00f5ff',
    visual: 'data',
  },
  {
    ...
    caseStudy: 'The page balances emotional brand positioning with conversion-focused sections: galleries, service details, trust cues, testimonials and WhatsApp-led enquiry. The result is a polished portfolio that feels editorial while still guiding users toward availability checks.',
    stack: ['HTML', 'CSS', 'JavaScript', 'GitHub Pages', 'Responsive Design', 'WhatsApp CTA'],
    color: '#a855f7',
    visual: 'xr',
  },
  {
    ...
    caseStudy: 'The build organizes faith, camp and contact information into a simple guided experience, supported by a Python/FastAPI backend and PostgreSQL data layer for dynamic content and structured administration. Strong navigation, repeated calls to action and clear content blocks help visitors understand the organization and find the next relevant step quickly.',
    stack: ['Python', 'FastAPI', 'PostgreSQL', 'HTML', 'CSS', 'JavaScript', 'Responsive Layout', 'Domain Hosting'],
    color: '#7ef7c9',
    visual: 'neural',
  },
  {
    ...
    caseStudy: 'The project centers on trust, clarity and fast access to business information. The experience is structured for customers who need product confidence, store details and a polished first impression without friction.',
    stack: ['HTML', 'CSS', 'JavaScript', 'Responsive UI', 'SEO Basics', 'Domain Setup'],
    color: '#ff9a3d',
    visual: 'void',
  },
]
    ...
    <section className="section work depth-section" id="work" ref={sectionRef}>
      ...
      <div className="work__inner">
        ...
        <div className="work__layout">
              ...
              >
                <div className="work__project-inner">
                  <div className="work__project-meta">
                    <span className="text-mono" style={{ color: 'var(--white-dim)', fontSize: '0.65rem' }}>{proj.id}</span>
                    <span className="text-mono" style={{ color: 'var(--white-dim)', fontSize: '0.65rem' }}>{proj.year}</span>
                  </div>
                  <h3 className="work__project-title" style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.5rem, 3vw, 2.2rem)', fontWeight: 700 }}>
                    {proj.title}
                  </h3>
                  <span className="text-mono work__project-type" style={{ fontSize: '0.7rem', color: proj.color }}>
                    {proj.type}
                  </span>
                  <p className="work__project-desc text-body">{proj.desc}</p>
                  <div className="work__project-stack" aria-label={`${proj.title} tech stack`}>
                    {proj.stack.slice(0, 3).map((tech) => (
                      <span key={tech}>{tech}</span>
                    ))}
                  </div>
                </div>
                <div className="work__project-bar" style={{ background: proj.color }} />
          ...
          <div className="work__preview">
            <div className="work__preview-inner">
              <div className="work__preview-visual" style={{ '--accent': projects[active].color }}>
                <img
                ...
                <div className="work__preview-label">
                  <span className="text-mono" style={{ fontSize: '0.65rem', letterSpacing: '0.15em' }}>
                    {projects[active].id} / PREVIEW
                  </span>
                </div>
              </div>
              <div className="work__preview-info" style={{ '--accent': projects[active].color }}>
                <div className="work__case-header">
                  <span className="text-mono">Case Study</span>
                  <a className="work__case-link hoverable" href={projects[active].url} target="_blank" rel="noreferrer">
                    Visit Site
                  </a>
                </div>
                ...
                <div className="work__tech-stack" aria-label={`${projects[active].title} full tech stack`}>
                  {projects[active].stack.map((tech, index) => (
                    <span key={tech} style={{ '--i': index }}>{tech}</span>
                  ))}
                </div>

                <button className="btn btn-outline hoverable" type="button" onClick={() => openProject(projects[active].url)} style={{ marginTop: '1rem', fontSize: '0.7rem', padding: '0.6rem 1.2rem', borderColor: projects[active].color, color: projects[active].color }}>
                  Open Project
                </button>
              </div>
            </div>
        ...
        <div className="work__cta">
          <p className="text-body" style={{ color: 'var(--white-dim)' }}>Ready to build something extraordinary?</p>
          <button className="btn btn-primary hoverable" type="button" onClick={scrollToContact} style={{ marginTop: '1rem' }}>Start a Project</button>
        </div>
      </div>
    </section>
  )
