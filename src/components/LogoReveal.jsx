...
export default function LogoReveal() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      function animateLogo() {
        ...
        tl.from('.logo-reveal__tagline span', {
          opacity: 0,
          y: 20,
          stagger: 0.05,
          duration: 0.6,
        ...
        tl.from('.logo-reveal__underline', {
          scaleX: 0,
          duration: 1.2,
          ease: 'power3.inOut'
        }, '-=0.8')

        tl.from('.logo-reveal__footer', {
          opacity: 0, y: 30, stagger: 0.1, duration: 0.6
        }, '-=0.4')

        tl.from('.logo-reveal__contact', {
  ...
  return (
    <section className="section logo-reveal depth-section" id="contact" ref={sectionRef}>
      ...
      <div className="logo-reveal__content">
        ...
        <div className="logo-reveal__contact">
          <div className="logo-reveal__contact-inner">
            <div className="logo-reveal__contact-col">
              <span className="text-mono" style={{ color: 'var(--cyan)', fontSize: '0.65rem' }}>READY TO BUILD?</span>
              <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.5rem, 3vw, 2.5rem)', fontWeight: 700, marginTop: '0.5rem' }}>
                Let's Create<br /><span className="grad-full">Something Legendary</span>
              </h3>
            </div>
            <div className="logo-reveal__contact-actions">
              <button className="btn btn-primary hoverable" type="button" onClick={() => window.location.href = 'mailto:hello@nxwstudios.ai?subject=New%20NXW%20project'}>
                Start a Project
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path d="M3 8h10M8 3l5 5-5 5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </button>
              <a href="mailto:hello@nxwstudios.ai" className="btn btn-outline hoverable">
                hello@nxwstudios.ai
              </a>
            </div>
          </div>
        ...
        <footer className="logo-reveal__footer-bar">
          <div className="logo-reveal__footer">
            <span className="text-mono" style={{ fontSize: '0.65rem', color: 'var(--white-dim)' }}>
